...
<Steps>
  ...
  <Step>
    **Early exit** — the moment a group's condition is `false`, all pending
    results beyond that point are discarded. See [When a condition
    fails](#when-a-condition-fails) to keep results that already arrived.
  </Step>
</Steps>
...
| **Skip group and keep checking**     | Discard this group, then evaluate the remaining conditions as normal. A later group is included only if its condition passes and it has already finished. |
| **Keep everything already finished** | Include this group and every group after it that has already finished, ignoring their conditions.                                                         |
